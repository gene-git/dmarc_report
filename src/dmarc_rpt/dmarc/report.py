...
from ._class_dmarc import (IPRpt, DmarcRpt, OrgRpt, DomRpt)
from .report_colors import (dmarc_colors, dkim_colors, spf_colors)
...
def print_report_header(cols: ColWidth):
    """
...
def print_total(rpt: DmarcRpt, name: str, tot: IPRpt, cols: ColWidth,
                do_dashes=True):
    """
...
def _format_item(prnt: Prnt, width: int, color: str, txt: str):
    """
    Colorize text.

    handle width adjustments from ascii color codes in output string.
    """
    (txt_fmt, clen) = prnt.colorize(txt, fg_col=color)
    wid = width + clen
    txt_fmt = f'{txt_fmt:>{wid}s}'
    return txt_fmt


def print_ip_row(rpt: DmarcRpt, name: str, iprpt: IPRpt, cols: ColWidth):
    """
    ...
    # DKIM column
    #
    (color_pass, color_fail, color_align) = \
        dkim_colors(cnt, dkim_auth_pass,
                    dkim_auth_fail, dkim_policy_pass)
    ...
    #
    (color_pass, color_fail, color_align) = \
        spf_colors(cnt, spf_auth_pass, spf_auth_fail,
                   spf_policy_pass)
...
def print_domain_report(rpt: DmarcRpt, org: OrgRpt, dom: DomRpt,
                        cols: ColWidth):
    """
    Server -> Domain
                  |   dmarc         | spf              | dkim
...
def print_report(rpt: DmarcRpt):
    ...
    print(f'{"":5s} {"Short":^6s} {"Selector":20s} {"Pass":>6s} \
            {"Fail":>6s}   {"DKIM domain":20s}')
    print(f'{"":5s} {dash6:^6s} {dash20:20s} {dash6:>6s} \
            {dash6:>6s}   {dash20:20s}')
    for sel in sel_map.selectors:
        ...
        print(f'{"":5s} {sel.short:^6s} {sel.name:20s} \
                {pass_s} {fail_s}   {sel_domains}')
